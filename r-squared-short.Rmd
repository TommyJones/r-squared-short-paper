...
The definitions in In (\ref{eqn:r2def1}) and (\ref{eqn:r2def2}) are "model free". But for a linear model, they are alegebraically equivalent to

\begin{equation}
  \label{eqn:r2var}
    R^2 
      = \frac{V_{i=1}^N \hat{y}_i}{V_{i=1}^N y_i}
\end{equation} 

where $V_{i=1}^N z_i = \frac{1}{N - 1} \sum_{i=1}^N (z_i - \bar{z})^2$. Equation (\ref{eqn:r2var}) is the proportion of variance in $y$ accounted for by the (linear) model. Researchers have used and extended (\ref{eqn:r2var}) to develop $R^2$ metrics for a range of models such as logistic regression [@hu2006], mixed models [@piepho2019], Bayesian regression models [@gelman2018r2], and more.
...
```{r geometric_graphic, fig.cap = "Visualizing the geometric interpretation of R-squared. Sum up the squared length of each line segment. This figure corresponds to an R-squared of 0.87", fig.asp = .55}
set.seed("8675309")
...
# Discussion
We have shown that $R^2$ can be calculated for multivariate outcomes from a geometric interpretation of (\ref{eqn:r2def1}).

A geometric interpretation of traditional $R^2$ lends itself to an ex

* Extend R^2 to n-dimensional outcomes using geometric interpretation of traditional definition
* Does not alter existing issues motivating alternate definitions for e.g. bayesian models, nonlinear models
* Worth noting: no lower bound of zero and squared euclidean distances
* Made available in R package mvrsquared
* Future research may explore different distance functions--as different variances for different models
