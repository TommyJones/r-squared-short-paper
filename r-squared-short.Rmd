...
abstract: >
  This research proposes a new (old) metric for evaluating goodness of fit. The cannonical coefficient of determination, $R^2$, is useful only for outcomes in one dimension. We propose an extension of the traditional $R^2$ valid for outcomes in multiple dimensions, when one is simultaneously predicting multiple variables. This metric is included in an R package on CRAN, `mvrsquared`. We demonstrate its use in several contexts: responses with 2 and 4 dimensions, function-on-function regression, and probabilistic topic modeling. 
draft: true
author: 
  - name: Tommy Jones
